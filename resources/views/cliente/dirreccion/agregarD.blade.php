...
<form class="Direc-mar"
    action="{{ route ('DirecUpdate',['tipo'=>'update','id'=>$id ]) }}"
    method="POST">
            ...
            <div class="direccion-add">
                <div class="Alias">
                    ...
                    <input type="text" name="alias" id="alias"
                        value="{{ isset($alias) ? $alias : old('alias') }}">
                </div>
                <div class="Espacio">
                    @phpif(isset($_GET['create'])){
                    @endphp
                ...
                <div class="Ciudad">
                    ...
                    <input type="text" name="ciudad" id="ciudad"
                        value="{{ isset($ciudad) ? $ciudad : old('alias') }}">
                </div>
                <div class="CodigoPost">
                    <label for="CP"> Codigo Postal </label>
                    <input type="text" name="CP" id="CP"
                        value="{{ isset($cp) ? $cp : old('alias') }}">
                </div>
                <div class="Colonia">
                    <label for="colonia"> Colonia </label>
                    <input type="text" name="colonia" id="colonia"
                        value="{{ isset($colonia) ? $colonia: old('alias') }}">
                </div>
                <div class="Referencia">
                    <label for="Refe"> Municipio o Delegación </label>
                    <input type="text" name="MuDe" id="MuDe"
                        value="{{ isset($muni) ? $muni : old('alias') }}">
                    <input type="hidden" name="create"
                        value="{{ (isset($_GET['create'])) ? 1:0 }}">
                </div>
                <div class="boton">
